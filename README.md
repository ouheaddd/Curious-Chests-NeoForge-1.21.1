...
- **Bottomless Chest** — 63-slot bulk storage. Opening costs 7 experience points.
...
- `client/` — menu screen registration and placeholder GUI rendering
- `mixin/` — reserved compatibility area; no mixins are currently required

## Placeholder art

Current textures and models are deliberately simple placeholders. Replace files under `assets/curiouschests/textures/` and the matching model JSON files without changing registry IDs.

## Current implementation notes

- Bottomless Chest currently uses 63 ordinary Minecraft slots. A deep-stack backend can replace this later without changing its registry ID.
- Ender Dispatch targets nearby block entities implementing vanilla `Container`, skips furnaces and Curious Chests blocks, does not load chunks, and does not cross dimensions.
